...
const PuzzlePiece : React.FC<PuzzlePieceProps> = ({ symbol, status, index }) => {
    return (
        <div className="flex justify-center items-center bg-white dark:bg-slate-500 rounded-md border border-black/5 dark:border-slate-600 border-b-4 flex-1 md:max-w-[3.5em] max-w-[3em] h-[3em] md:h-14 m-1">
            <div 
            style={{ 
             ...
             className="flex justify-center bg-white dark:bg-slate-500 items-center flex-1 w-full h-full m-1 rounded-md">
                <h1    
                    style={{ 
                        transform: status === ESymbolStatus.UNKNOWN ? "rotateX(0deg)" : "rotateX(180deg)",
